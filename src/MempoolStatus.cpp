...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        last_tx.fee_str         = inceg::inc_amount_to_str(_tx_info.fee, "{:0.3f}", false);
        last_tx.inc_inputs_str  = inceg::inc_amount_to_str(last_tx.sum_inputs , "{:0.3f}");
        last_tx.inc_outputs_str = inceg::inc_amount_to_str(last_tx.sum_outputs, "{:0.3f}");
        last_tx.timestamp_str   = inceg::timestamp_to_str_gm(_tx_info.receive_time);
...
bf::path MempoolStatus::blockchain_path {"/home/mwo/.incognito/lmdb"};
string MempoolStatus::deamon_url {"http:://127.0.0.1:39002"};
bool   MempoolStatus::testnet {false};
atomic<bool>       MempoolStatus::is_running {false};
boost::thread      MempoolStatus::m_thread;
Blockchain*        MempoolStatus::core_storage {nullptr};
inceg::MicroCore*  MempoolStatus::mcore {nullptr};
