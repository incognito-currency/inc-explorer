...
#ifndef CROWINC_RPCCALLS_H
#define CROWINC_RPCCALLS_H
...
{
// declare struct in incognito's cryptonote namespace.
// incognito should provide definition for this,
...
public:
    ...
    rpccalls(string _deamon_url = "http:://127.0.0.1:39002",
             uint64_t _timeout = 200000);

    bool
    connect_to_incognito_deamon();
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
...

        {
            ...
            if (!connect_to_incognito_deamon())
            {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Incognito deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Incognito deamon at "
                 << deamon_url << endl;
            return false;
        }
