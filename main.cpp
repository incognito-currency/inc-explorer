...
main(int ac, const char* av[])
{
    ...
    inceg::CmdLineOptions opts {ac, av};
    ...
    // set  incognito log output level
    ...
    if (!inceg::get_blockchain_path(bc_path_opt, blockchain_path, testnet))
    {
    ...
    inceg::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!inceg::init_blockchain(blockchain_path.string(),
                               mcore, core_storage))
    {
    ...
    if (testnet && deamon_url == "http:://127.0.0.1:39002")
    {
        deamon_url = "http:://127.0.0.1:29002";
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        // e.g., ~/.incognito/lmdb/emission_amount.txt.
        ...
        inceg::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        inceg::CurrentBlockchainStatus::testnet
                = testnet;
        inceg::CurrentBlockchainStatus::deamon_url
                = deamon_url;
        inceg::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        inceg::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    inceg::MempoolStatus::blockchain_path
            = blockchain_path;
    inceg::MempoolStatus::testnet
            = testnet;
    inceg::MempoolStatus::deamon_url
            = deamon_url;
    inceg::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    inceg::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    inceg::MempoolStatus::start_mempool_status_thread();

    // create instance of page class which
    // contains logic for the website
    inceg::page incblocks(&mcore,
                          core_storage,
                          deamon_url,
                          testnet,
                          enable_pusher,
    ...
    ([&](const crow::request& req) {
        return crow::response(incblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return incblocks.index2(page_no);
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](const crow::request& req, size_t block_height) {
        return crow::response(incblocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](const crow::request& req, string block_hash) {
        return crow::response(incblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](const crow::request& req, string tx_hash) {
        return crow::response(incblocks.show_tx(remove_bad_chars(tx_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>/<uint>")
    ([&](string tx_hash, uint16_t with_ring_signatures)
     {
        return incblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures);
    });
    ...
    ([&](const crow::request& req)
     {
        ...
        map<std::string, std::string> post_body
                = inceg::parse_crow_post_data(req.body);

        if (post_body.count("inc_address") == 0
            || post_body.count("viewkey") == 0
            || post_body.count("tx_hash") == 0)
        {
            return string("inc address, viewkey or tx hash not provided");
        }

        string tx_hash     = remove_bad_chars(post_body["tx_hash"]);
        string inc_address = remove_bad_chars(post_body["inc_address"]);
        ...
        return incblocks.show_my_outputs(tx_hash, inc_address,
                                         viewkey, raw_tx_data,
                                         domain);
    });
    ...
    ([&](const crow::request& req, string tx_hash,
        string inc_address, string viewkey)
     {

        string domain = get_domain(req);

        return incblocks.show_my_outputs(remove_bad_chars(tx_hash),
                                         remove_bad_chars(inc_address),
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            if (post_body.count("incaddress") == 0
                || post_body.count("txprvkey") == 0
                || post_body.count("txhash") == 0)
            {
                return string("inc address, tx private key or "
            ...
            string inc_address = remove_bad_chars(post_body["incaddress"]);
            ...
            return incblocks.show_prove(tx_hash,
                                        inc_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain);
    ...
    ([&](const crow::request& req, string tx_hash,
         string inc_address, string tx_prv_key) {

        string domain = get_domain(req);

        return incblocks.show_prove(remove_bad_chars(tx_hash),
                                    remove_bad_chars(inc_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&](const crow::request& req) {
            ...
            if (action == "check")
                return incblocks.show_checkrawtx(raw_tx_data, action);
            else if (action == "push")
                return incblocks.show_pushrawtx(raw_tx_data, action);
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&](const crow::request& req) {
            return incblocks.show_rawkeyimgs();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return incblocks.show_checkrawkeyimgs(raw_data, viewkey);
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&](const crow::request& req) {
            return incblocks.show_rawoutputkeys();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return incblocks.show_checkcheckrawoutput(raw_data, viewkey);
        });
    }
    ...
    ([&](const crow::request& req) {
        return incblocks.search(remove_bad_chars(string(req.url_params.get("value"))));
    });

    CROW_ROUTE(app, "/mempool")
    ([&](const crow::request& req) {
        return incblocks.mempool(true);
    });
...
//    });
    ...
    if (enable_json_api)
    {
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            myinc::jsonresponse r{incblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            myinc::jsonresponse r{incblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            myinc::jsonresponse r{incblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            myinc::jsonresponse r{incblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            myinc::jsonresponse r{incblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            myinc::jsonresponse r{incblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](const crow::request &req, string search_value) {
            ...
            myinc::jsonresponse r{incblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&](const crow::request &req) {
            ...
            myinc::jsonresponse r{incblocks.json_networkinfo()};
        ...
        ([&](const crow::request &req) {
            ...
            myinc::jsonresponse r{incblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            myinc::jsonresponse r{incblocks.json_outputs(
        ...
        ([&](const crow::request &req) {
            ...
            myinc::jsonresponse r{incblocks.json_outputsblocks(
        ...
        ([&](const crow::request &req) {
            ...
            myinc::jsonresponse r{incblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            ...
            return incblocks.index2(page_no, refresh_page);
        });
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        inceg::CurrentBlockchainStatus::m_thread.interrupt();
        inceg::CurrentBlockchainStatus::m_thread.join();
    ...
    inceg::MempoolStatus::m_thread.interrupt();
    inceg::MempoolStatus::m_thread.join();
