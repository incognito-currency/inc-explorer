# Incognito Blockchain Explorer

A Blockchain Explorer for Incognito currency.
 
## Incognito Blockchain Explorer features

The key features of the Incognito Blockchain Explorer are:
 ...
 - showing public components of Incognito addresses,
 - decoding which outputs and mixins belong to the given Incognito address and viewkey,
 - can prove that you send Incognito to someone,
 - detailed information about mixins, such as, mixins'
 age, timescale, mixin of mixins,
 - showing number of amount output indices,
 - support Incognito testnet network,
...
##### Compile latest Incognito development version

Download and compile recent Incognito into your home folder:

```bash
# first install incognito dependecines
sudo apt update
...
git clone https://github.com/incognito-currency/incognito
...
Once the Incognito is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/incognito
cd ~

# download the source code
git clone https://github.com/incognito-currency/inc-explorer.git
...
# altearnatively can use: cmake -DINCOGNITO_DIR=/path/to/incognito_folder .. 
# if incognito is not in ~/incognito
#
...
When compilation finishes executable `incblocks` should be created. Before running
...
By default it will look for blockchain in its default location i.e., `~/.incognito/lmdb`.
You can use `--bc-path` option if its in different location. 
Example output:

```bash
[mwo@arch inc-explorer]$ ./incblocks
2016-May-28 10:04:49.160280 Blockchain initialized. last block: 1056761, d0.h0.m12.s47 time ago, current difficulty: 1517857750
(2016-05-28 02:04:49) [INFO    ] Crow/0.1 server is running, local port 9002
```

Go to your browser: http://127.0.0.1:9002
...
incblocks, Incognito Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Incognito total emission monitoring
                                        thread
  -p [ --port ] arg (=9002)             default explorer port
  ...
  --ssl-key-file arg                    path to key file for ssl (https) 
                                        functionality
  -d [ --deamon-url ] arg (=http:://127.0.0.1:39002)
...
alias incblocksmainnet='~/inc-explorer/build/incblocks    --port 9002 --testnet-url "http://139.162.32.245:9003" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias incblockstestnet='~/inc-explorer/build/incblocks -t --port 9003 --mainnet-url "http://139.162.32.245:9002" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:9002/ and http://139.162.32.245:9003/, respectively.

## Enable Incognito emission 

Obtaining current Incognito emission amount is not straight forward. Thus, by default it is 
disabled. To enable it use `--enable-emission-monitor` flag, e.g., 


```bash
incblocks --enable-emission-monitor 
```
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.incognito/lmdb/emission_amount.txt`. For testnet network, 
 it is `~/.incognito/testnet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.incognito/lmdb/emission_amount.txt`
...
```
Incognito emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Incognito daemon's  `print_coinbase_tx_sum` command. 
...
Having the `crt` and `key` files, run `incblocks` in the following way:

```bash
./incblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key 
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/transaction/6093260dbe79fd6277694d14789dc8718f1bd54457df8bab338c2efa3bb0f03d"
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/transactions"
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/transactions?page=2&limit=10"
```
...
```bash
curl  -w "\n" -X GET "http://139.162.32.245:9002/api/block/1293257"
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/mempool"
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/mempool?limit=10"
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/search/1293669"
```
...
```bash
# we use here official Incognito project's donation address as an example
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/outputs?txhash=17049bc5f2d9fbca1ce8dae443bbbbed2fc02f1ee003ffdd0571996905faa831&address=incGc73jTiyXi5uUJ29aFb71DMkgeMRCAYoXjkRStHT14oR1d1KAnaKTBuzpjqZmSccyfHUuETNHoaTLj93doB6E9pE2j6zGfa&viewkey=f359631075708155cc3d92a32b75a7d02a5dcf27756707b47a2b31b21c389501&txprove=0"
```

```json
{
  "data": {
    "address": "incGc73jTiyXi5uUJ29aFb71DMkgeMRCAYoXjkRStHT14oR1d1KAnaKTBuzpjqZmSccyfHUuETNHoaTLj93doB6E9pE2j6zGfa",
    "outputs": [
      {
      ...
      },
      {
        ...
        "match": false,
        "output_idx": 1,
        "output_pubkey": "incGc73jTiyXi5uUJ29aFb71DMkgeMRCAYoXjkRStHT14oR1d1KAnaKTBuzpjqZmSccyfHUuETNHoaTLj93doB6E9pE2j6zGfa"
      }
    ],
...
We use recipient's address (i.e. not our address from which we sent inc to recipient).
...
curl  -w "\n" -X GET "http://127.0.0.1:9003/api/outputs?txhash=94782a8c0aa8d8768afa0c040ef0544b63eb5148ca971a024ac402cad313d3b3&address=9wUf8UcPUtb2huK7RphBw5PFCyKosKxqtGxbcKBDnzTCPrdNfJjLjtuht87zhTgsffCB21qmjxjj18Pw7cBnRctcKHrUB7N&viewkey=e94b5bfc599d2f741d6f07e3ab2a83f915e96fb374dfb2cd3dbe730e34ecb40b&txprove=1"
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/networkinfo"
```
...
```json
{
  "data": {
    ...
    "target_height": 0,
    "testnet": false,
    "top_block_hash": "76f9e85d62415312758bc09e0b9b48fd2b005231ad1eee435a9002e551203f82",
    "tx_count": 1219048,
    "tx_pool_size": 2,
...
```bash
# testnet address
curl  -w "\n" -X GET http://127.0.0.1:9002/api/outputsblocks?address=9sDyNU82ih1gdhDgrqHbEcfSDFASjFgxL9B9v5f1AytFUrYsVEj7bD9Pyx5Sw2qLk8HgGdFM8qj5DNecqGhm24Ce6QwEGDi&viewkey=807079280293998634d66e745562edaaca45c0a75c8290603578b54e9397e90a&limit=5&mempool=1
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/emission"
```
...
```bash
curl  -w "\n" -X GET "http://127.0.0.1:9002/api/version"
```
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_incognito",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "incognito_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Incognito.

```bash
curl  -w "\n" -X GET "http://139.162.32.245:9002/api/rawblock/1293257"
```
...
Return raw json tx data, as represented in Incognito.

```bash
curl  -w "\n" -X GET "http://139.162.32.245:9002/api/rawtransaction/6093260dbe79fd6277694d14789dc8718f1bd54457df8bab338c2efa3bb0f03d"
```
